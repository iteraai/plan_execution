...
def write_install_context(runtime_dir: Path, *, target: InstallTarget) -> None:
    (runtime_dir / "install_context.py").write_text(
        "\n".join(
            [
                '"""Generated by install.py; records the agent target for runtime defaults."""',
                "",
                f"INSTALL_TARGET = {json.dumps(target.name)}",
                "",
            ]
        )
    )
